        ...
        switch ($endpoint) {
            case 'posts':
                if ($id) {
                    ...
                    // Use direct SQL without parameter binding for LIMIT/OFFSET
                    $sql = "
                        SELECT p.*, a.name as authorName, a.email as authorEmail, a.avatar as authorAvatar,
                               c.name as categoryName, c.slug as categorySlug
                        FROM post p 
...
function handlePost($pdo, $endpoint) {
    ...
    try {
        switch ($endpoint) {
            case 'post-view':
                // Track post view - check if table exists first
                try {
                    ...
                    // Table might not exist, create simple response
